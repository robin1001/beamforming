...
static void delay_sequence(float *in_data, int num, int delay, float *out_data) {
    for (int i = 0; i < num; i++) {
...
int main() {
    ...
    delay_sequence(g_data, NUM_DATA, 10, delay_data1);
    delay_sequence(g_data, NUM_DATA, -5, delay_data2);
    ...
    calc_tdoa(all_data, NUM_CHANNEL, NUM_DATA, 0, NUM_DATA / 2, tdoa);
    ...
    delay_and_sum(all_data, NUM_CHANNEL, NUM_DATA, tdoa, out);
